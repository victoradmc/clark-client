...
export default function AdminScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (profile.role === "admin") await loadAccounts();
      } catch {
  ...
  // Shared shape for every per-row action below: clear that row's error,
  // run the action, and on failure surface the server's message under that
  // same row rather than a screen-wide error (each row acts independently).
